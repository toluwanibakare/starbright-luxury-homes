...
const Listings = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (location !== "All Locations") result = result.filter((l) => l.location === location);
      ...
      <div className="pt-28 pb-12 section-padding !pb-8" style={{ background: "var(--gradient-subtle)" }}>
        <div className="container-premium">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <p className="text-xs uppercase tracking-[0.3em] text-primary font-semibold mb-2">Explore</p>
            <h1 className="text-3xl md:text-4xl font-bold text-foreground font-display mb-2">Property Listings</h1>
            <p className="text-muted-foreground">Browse our collection of verified properties.</p>
          </motion.div>
        </div>
      </div>

      <div className="container-premium px-4 sm:px-6 lg:px-8 py-8">
        {/* Search & Filter Bar */}
        <div className="flex flex-col sm:flex-row gap-3 mb-8">
          ...
          <button
            ...
            className="premium-btn-outline flex items-center gap-2 text-xs !py-3 sm:w-auto"
          >
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                ...
                <select value={location} onChange={(e) => setLocation(e.target.value)} className="hero-search-input w-full">
                  {locations.map((l) => <option key={l}>{l}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">Category</label>
                <select value={category} onChange={(e) => setCategory(e.target.value)} className="hero-search-input w-full">
              ...
              <div>
                ...
                <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="hero-search-input w-full">
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">Verified</label>
